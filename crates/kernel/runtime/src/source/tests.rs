...
#[test]
fn context_series_source_reads_context_value_default_key() {
    let source = ContextSeriesSource::new();
    let ctx = ExecutionContext::from_values(HashMap::from([(
        "x".to_string(),
...
#[test]
fn context_series_source_reads_context_value_custom_key() {
    let source = ContextSeriesSource::new();
    let ctx = ExecutionContext::from_values(HashMap::from([(
        "samples".to_string(),
        Value::Series(vec![5.0, 8.0]),
    )]));

    let outputs = source.produce(
        &HashMap::from([(
            "key".to_string(),
            crate::source::ParameterValue::String("samples".to_string()),
        )]),
        &ctx,
    );
    assert_eq!(outputs.get("value"), Some(&Value::Series(vec![5.0, 8.0])));
}

#[test]
fn context_series_source_missing_key_returns_default_empty_series() {
    let source = ContextSeriesSource::new();
    let ctx = ExecutionContext::from_values(HashMap::from([(
        "other".to_string(),
        Value::Series(vec![99.0]),
    )]));

    let outputs = source.produce(&HashMap::new(), &ctx);
    assert_eq!(outputs.get("value"), Some(&Value::Series(vec![])));
}

#[test]
fn context_series_source_wrong_type_returns_default_empty_series() {
    let source = ContextSeriesSource::new();
    let ctx = ExecutionContext::from_values(HashMap::from([(
        "x".to_string(),
        Value::String("not a series".to_string()),
    )]));
...
#[test]
fn context_series_source_manifest_with_dollar_key_validates() {
    let manifest = context_series_source_manifest();
    assert!(SourceRegistry::validate_manifest(&manifest).is_ok());
    ...
    let key_param = manifest
        ...
        .iter()
        .find(|p| p.name == "key")
        .expect("context_series_source must declare 'key' parameter");
    assert_eq!(key_param.value_type, ParameterType::String);
    assert_eq!(
        key_param.default,
        Some(crate::source::ParameterValue::String("x".to_string()))
