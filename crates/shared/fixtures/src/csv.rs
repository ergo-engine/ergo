...
#[derive(Debug, Clone)]
pub struct CsvToFixtureOptions {
    pub input: PathBuf,
    pub output: PathBuf,
...
pub fn parse_event_kind(raw: &str) -> Result<ExternalEventKind, String> {
    let normalized = raw
        .chars()
        .filter(|ch| *ch != '_' && *ch != '-')
        .collect::<String>()
...
pub fn convert_csv_to_fixture(options: &CsvToFixtureOptions) -> Result<usize, String> {
