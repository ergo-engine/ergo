...
//!
//! Purpose:
//! - Normalize shared raw graph authoring data into loader-owned `ClusterDefinition`
//!   values.
//! - Hold the shared decode-time structs and helpers used by both YAML and JSON graph parsers.
//!
//! Owns:
//! - Raw graph authoring structs/enums shared across YAML and JSON decoding.
//! - Shared identifier, version, edge-reference, and declared-signature parsing helpers.
//! - The shared `decode_raw_graph(...)` path that turns parsed authoring data into a decoded graph.
//!
//! Does not own:
//! - YAML or JSON string/file parsing, source labeling, or dual-format fallback selection.
//! - Filesystem discovery, in-memory source resolution, or kernel semantic validation.
//!
//! Connects to:
//! - `yaml_graph.rs` and `json_graph.rs`, which parse transport-specific text into these raw
//!   authoring shapes.
//! - `decode/mod.rs` and `discovery.rs`, which reuse the shared identifier and selector helpers.
//!
//! Safety notes:
...
fn parse_output_endpoint_ref(raw: RawEndpointRef) -> Result<(OutputRef, Option<String>), String> {
    match raw {
        RawEndpointRef::NodePort { node, port } => {
            ...
            validate_node_port_reference_segment(&port, "edge source port")?;
            Ok((
                OutputRef {
                    node_id: node,
                    port_name: port,
...
fn parse_input_endpoint_ref(raw: RawEndpointRef) -> Result<InputRef, String> {
    match raw {
        RawEndpointRef::NodePort { node, port } => {
            ...
            validate_node_port_reference_segment(&port, "edge target port")?;
            Ok(InputRef {
                node_id: node,
                port_name: port,
            })
...
fn parse_node_port_ref(value: &str, label: &str) -> Result<(String, String), String> {
    ...
    validate_node_port_reference_segment(port, &format!("{label} port"))?;
