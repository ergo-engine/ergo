...
| **`ergo-sdk-rust`**     | `crates/prod/clients/sdk-rust`    | The SDK `Ergo` engine, `ErgoBuilder`, `ProfileRunner`, `StopHandle`, SDK-branded `Ergo*Error` taxonomy, in-memory project plumbing.                             | Host orchestration internals; loader internals; kernel semantics.             | `ergo-adapter`, `ergo-host`, `ergo-loader`, `ergo-runtime`.                                               |
...
```
        ...
        └───────┬───────┘
                │
        ┌───────▼───────┐
        │ ergo-sdk-rust │  (library facade for embedded callers)
        └───────┬───────┘
