...
pub fn validate_command(args: &[String]) -> Result<String, String> {
    ...
    let parsed = parse_manifest(path).map_err(|err| render_failure(&[err], format))?;
    let summary = parsed.summary().clone();
    match validate_parsed(&parsed) {
        Ok(()) => Ok(render_success(&summary, format)),
        Err(err) => Err(render_failure(&[err], format)),
    }
}
...
pub fn check_compose_command(args: &[String]) -> Result<String, String> {
    ...
    let adapter_manifest = parse_adapter_manifest(adapter_path)
        .map_err(|msg| render_failure(&[parse_error_violation(msg)], format))?;
    ergo_adapter::validate_adapter(&adapter_manifest)
        .map_err(|err| render_failure(&[RuleViolation::from(err)], format))?;
    ...
    let result = match other {
        ...
        _ => {
            return Err(format!(
                "unsupported manifest kind for composition: '{}'",
                other.summary().kind
            ));
        }
    };
    ...
    match result {
        ...
        Err(err) => Err(render_failure_with_header(
            &[err],
            format,
            "Composition invalid",
        )),
...
fn render_failure(errors: &[RuleViolation], format: OutputFormat) -> String {
    render_failure_with_header(errors, format, "Manifest invalid")
}

fn render_failure_with_header(
...
fn render_failure_text(errors: &[RuleViolation], header: &str) -> String {
    let mut out = format!("✗ {}\n\n", header);
    for err in errors {
...
fn render_failure_json(errors: &[RuleViolation]) -> String {
...
fn rule_violation_to_json(err: &RuleViolation) -> serde_json::Value {
    serde_json::json!({
        "rule_id": err.rule_id,
        "phase": phase_name(err.phase),
        "doc_anchor": err.doc_anchor,
        "summary": err.summary.clone().into_owned(),
        "path": err.path.clone().map(|p| p.into_owned()),
        "fix": err.fix.clone().map(|f| f.into_owned()),
    })
}
