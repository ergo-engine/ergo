...
pub fn cli_error_from_error_info(
    code: impl Into<String>,
    message: impl Into<String>,
    where_field: impl Into<String>,
    err: &impl ErrorInfo,
) -> String {
    let mut info = CliErrorInfo::new(code, message)
        .with_rule_id(err.rule_id().to_string())
        .with_where(where_field.into())
        .with_detail(err.summary().to_string());

    if let Some(path) = err.path() {
        info = info.with_detail(format!("path: {path}"));
    }
    if let Some(fix) = err.fix() {
        info = info.with_fix(fix.to_string());
    }
