...
`resolve_impl_parameters()` in `crates/runtime/src/cluster.rs` (lines 1307-1347)
...
invariant in PHASE_INVARIANTS.md tracks this condition. No entry in closure_register.md.
...
4. A new invariant (e.g., `I.7`) in PHASE_INVARIANTS.md
5. Tests: `undeclared_parameter_binding_rejected`, `parameter_typo_rejected`

## 7. Disposition Options

- **A. Fix now.** Add the validation. Assign a rule ID. Document in PHASE_INVARIANTS.
- **B. Document as known gap.** Add to closure_register.md with V1 SEMANTICS disposition
...
5. Wired ErrorInfo: rule_id `"I.7"`, doc_anchor `STABLE/CLUSTER_SPEC.md#I.7`
6. Added I.7 to PHASE_INVARIANTS.md §2 (Instantiation Phase) — invariant #157
7. Added I.7 to CLUSTER_SPEC.md instantiation rules table
