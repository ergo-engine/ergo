...
- **Enforcement locus:** API removal + registry validation
  - `crates/kernel/runtime/src/trigger/mod.rs` (removed TriggerState + state param)
  - `crates/kernel/runtime/src/runtime/types.rs` (ExecutionContext is unit struct)
  - `crates/kernel/runtime/src/runtime/execute.rs` (removed state plumbing)
  - `crates/kernel/runtime/src/trigger/registry.rs` (kept `StatefulTriggerNotAllowed`)
...
- **Disposition:** CLOSE (defense-in-depth)
- **Enforcement locus:** test (`crates/kernel/runtime/src/catalog.rs`)
...
- **Disposition:** CLOSE
- **Enforcement locus:** runtime build path (`crates/kernel/runtime/src/catalog.rs`)
...
- **Disposition:** CLOSE
- **Enforcement locus:**
  - `crates/kernel/runtime/src/catalog.rs`: `CorePrimitiveCatalog::new()` and `register_compute/register_source/register_trigger/register_action` are `pub(crate)`.
  - `crates/kernel/adapter/src/lib.rs`: migrated test callsite to `build_core_catalog()` (no direct catalog construction/registration).
...
- **Enforcement locus (mapping correction):** `crates/kernel/runtime/src/common/errors.rs` (`InvalidParameterType` maps to `CMP-19`, not `CMP-15`).
- **Enforcement locus (runtime):** `crates/kernel/runtime/src/compute/registry.rs` parameter validation loop (typed rejection via `ValidationError::InvalidParameterType` when default type mismatches).
...
- **Follow-up status (resolved):** CLI parse-time default coercion failures now map to concrete per-kind rule IDs via typed parse errors in `crates/prod/clients/cli/src/validate.rs` (`CMP-19`, `SRC-15`, `TRG-14`, `ACT-19`) instead of `rule_id = INTERNAL`.
- **PR/Commit:** working tree (uncommitted)
...
- **Disposition:** CLOSE
- **Enforcement locus:** `crates/kernel/runtime/src/source/registry.rs` — parameter validation loop in `validate_manifest()`. Error: `SourceValidationError::InvalidParameterType { parameter, expected, got }`.
- **Rule ID mapping:** `crates/kernel/runtime/src/source/mod.rs` — `Self::InvalidParameterType { .. } => "SRC-15"`.
...
- **Disposition:** CLOSE
- **Enforcement locus:** `crates/kernel/runtime/src/trigger/registry.rs` — parameter validation loop in `validate_manifest()`. Error: `TriggerValidationError::InvalidParameterType { parameter, expected, got }`.
- **Rule ID mapping:** `crates/kernel/runtime/src/trigger/mod.rs` — `Self::InvalidParameterType { .. } => "TRG-14"`.
- **Path:** `$.parameters[].default`
- **Fix:** Change parameter default value to match the declared parameter type.
...
- **Disposition:** CLOSE
- **Enforcement locus:** `crates/kernel/runtime/src/action/registry.rs` — parameter validation loop in `validate_manifest()`. Error: `ActionValidationError::InvalidParameterType { parameter, expected, got }`.
- **Rule ID mapping:** `crates/kernel/runtime/src/action/mod.rs` — `Self::InvalidParameterType { .. } => "ACT-19"`.
- **Path:** `$.parameters[].default`
- **Fix:** Change parameter default value to match the declared parameter type.
...
- **Enforcement locus (mapping):** `crates/kernel/runtime/src/common/errors.rs` — `Self::InvalidOutputType { .. } => "CMP-20"`, with explicit `doc_anchor`, `path`, and `fix` entries.
- **Enforcement locus (registration):** `crates/kernel/runtime/src/compute/registry.rs` output validation loop includes exhaustive `ValueType` matching (`CMP-20` structural enforcement by type system).
- **Test (mapping):** `cmp_20_reserved_for_invalid_output_type` in `crates/kernel/runtime/src/common/errors.rs`.
- **Test (registration):** `cmp_20_output_types_valid` in `crates/kernel/runtime/src/compute/registry.rs`.
...
- **Enforcement loci:** `rule_id()` matches in:
  - `crates/kernel/runtime/src/source/mod.rs`
  - `crates/kernel/runtime/src/trigger/mod.rs`
...
- **Disposition:** CLOSE
- **Enforcement locus:** constant (`crates/kernel/supervisor/src/lib.rs`)
...
- **Disposition:** CLOSE
- **Enforcement locus:** test (`crates/kernel/supervisor/tests/replay_harness.rs`)
...
- **Disposition:** CLOSE
- **Enforcement locus:** `crates/kernel/runtime/src/action/mod.rs`
...
- `crates/kernel/runtime/src/source/implementations/string/` (mod.rs, manifest.rs, impl.rs)
...
- Runtime contract authority is Rust types + ui-runtime.md
...
- I.6 is enforced in `crates/kernel/runtime/src/cluster.rs` via strict semver selector parsing and deterministic highest-satisfying resolution for primitive and nested cluster references.
