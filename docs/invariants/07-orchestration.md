...
- SDK-CANON-1: now exercised by `ergo-sdk`. SDK `run_profile`, `replay_profile`, `validate_project`, and `runner_for_profile` delegate canonical orchestration to host entrypoint APIs, with SDK validation delegating to the run-validation seam (`validate_run_graph_from_paths...`) rather than inventing an SDK-only validation lane; `ProfileRunner::finish()` delegates finalization through `finalize_hosted_runner_capture(...)`; `ergo init` scaffolds against that real surface rather than a placeholder.
