...
| `validate_graph` | Lower-level | Validate preloaded `PreparedGraphAssets` plus `LivePrepOptions` without starting egress or constructing a started session (`validate_graph_with_surfaces` is the injected-runtime variant) |
...
- RUN-CANON-2: `validate_live_runner_setup_from_assets(...)` scans adapter dependencies before execution and `ensure_adapter_requirement_satisfied(...)` rejects adapter-dependent graphs when `adapter_path` is absent. The host error surface reports this as `RUN-CANON-2` with offending required-context/write nodes when available.
